...
Login c=custservice.login(userId);
    ...
    public ResponseEntity<Login> changepassword(@RequestBody Login login) {
        Login n=custservice.changepassword(login);
        ResponseEntity re = new ResponseEntity<Login>( HttpStatus.OK);
        return re;
    }
    @GetMapping("/forgotpassword/{userId}")
    public ResponseEntity<Login> forgotpassword(@PathVariable int userId){
        Login c2 = custservice.forgotPassword(userId);
        ResponseEntity re = new ResponseEntity<Login>(c2, HttpStatus.OK);
        return re;
    }
